...
// COPY_LINES moved into the component
...
export function Montage() {
  ...
  const COPY_LINES = useMemo(() => [
    t('loading_1', 'Parsing 518,400 Saju patterns…'),
    t('loading_2', 'Syncing your energy with Seoul\'s coordinates…'),
    t('loading_3', 'Calibrating Five Element resonance…'),
    t('loading_4', 'Allocating your Korean identity…'),
    t('loading_5', 'Crystallising your parallel life…'),
  ], [t]);
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center"
      ...
      <motion.div className="absolute top-14 left-1/2" style={{ transform: 'translateX(-50%)' }}
        ...
        <span style={{
          ...
          color: '#3a3028', letterSpacing: '0.2em'
        }}>2 / 3</span>
      </motion.div>
        ...
        {ELEMENTS.map((el, i) => {
          ...
          return (
            <motion.div key={el.char}
              ...
              style={{
                width: 48, height: 48, top: '50%', left: '50%',
              ...
              <span style={{
                ...
                fontWeight: 400, color: el.color, lineHeight: 1
              }}>{el.char}</span>
              <span style={{
                fontFamily: 'Pretendard, sans-serif', fontSize: '7px',
                color: el.color, opacity: 0.7, marginTop: 1
              }}>{el.kr}</span>
            </motion.div>
          );
        })}
      ...
      <motion.p className="mt-5 px-8 text-center"
        ...
        {t('loading_sub', '당신의 한국 정체성을 깨우는 중…')}
      </motion.p>
    </div>
  );
}
