...
import { getDynamicNarrativeContent } from '../../lib/contentApi';
...
export const FUTURE_BY_STEM: Record<string, FutureChapter> = {
        '甲': {
                title: '숲을 이루는 시대', keyword: '創業 · 유산', peak: '40~55세',
        ...
        '乙': {
                title: '꽃이 피는 전성기', keyword: '開花 · 인정', peak: '35~50세',
...
export async function getNarrative(saju: SajuData, language: string = 'ko'): Promise<NarrativeResult> {
        ...
        try {
                // 2. Parallel Fetch from New Relational Tables
                const [
                        dbZodiac,
                        dbDayMaster,
                        dbScenario,
                        ...
                        supabase.from('saju_content_zodiac').select('trait').eq('animal', result.zodiac.animal).single(),
                ...
                if (dbZodiac.data) result.zodiac.trait = dbZodiac.data.trait;
                if (dbDayMaster.data) {
                        result.dayMaster = {
                                ...result.dayMaster,
                                ...dbDayMaster.data,
                                ...
                                luckyColorHex: dbDayMaster.data.lucky_color_hex,
                                luckyNumber: dbDayMaster.data.lucky_number
                        };
                }
                if (dbScenario.data) result.scenario = dbScenario.data;
                if (dbPast.data) result.pastData = dbPast.data;
                if (dbYouth.data) result.youthData = dbYouth.data;
                if (dbFuture.data) result.futureData = dbFuture.data;
                // Soulmate and Neighborhood are usually handled in identity object, but we could update here too
        } catch (err) {
                console.warn('[NarrativeDB] Error fetching relational content, using fallbacks', err);
        }
